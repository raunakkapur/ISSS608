...
d<-data%>%mutate(agegroup=case_when(age<30~"Below 30",
                                    ...
                                    age>=40 &age<50~"40-49",
                                    age>=50 ~"50 and above"))
x_selected<-d%>%select(participantId,householdSize,haveKids,agegroup,educationLevel)
...
ggplot(data = AgeGroup_EducationLevel,
       aes(x=factor(agegroup,levels=c("Below 30","30-39","40-49","50 and above")),y=percent,fill=educationLevel))+
  geom_col()
```
...
ggplot(data = AgeGroup_EducationLevel,
  ...
  theme_bw()+
  guides(fill = guide_legend(title = "Education Level"),shape = guide_legend(override.aes = list(size = 0.5)))+
  theme(plot.title = element_text(hjust = 0.5,size = 13),legend.title = element_text(size = 9),legend.text = element_text(size = 7))
...
```{r}
ggplot(data = d) +
  geom_mosaic(aes(x = product(agegroup), fill=educationLevel)) +
...
annotate_figure(c,top = text_grob("Doughnut vs Bar", 
              face = "bold", size = 14))
```
...
ggplot(data=d,aes(y=joviality*100,x=Kids_Reordered))+
  ...
  xlab("")+
  ylab("Joviality (*0.01)")+
  theme(axis.text.x  = element_text(angle=0),plot.title = element_text(hjust = 0.5,size = 10))+
